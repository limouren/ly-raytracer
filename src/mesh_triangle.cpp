...
void MeshTriangle::buildBoundingBox() {
...
int MeshTriangle::intersect(const Ray &ray, Intercept intercepts[]) const {
...
std::vector<P_FLT> MeshTriangle::inverseMap(const Point3D &point) const {
...
void TexMeshTriangle::getIntersect(const Point3D &point, Vector3D * normal,
...
void PatchMeshTriangle::getIntersect(const Point3D &point, Vector3D * normal,
                                     std::vector<P_FLT> * mapping) const {
  *mapping = inverseMap(point);
...
void PhongMeshTriangle::getIntersect(const Point3D &point, Vector3D * normal,
                                     std::vector<P_FLT> * mapping) const {
  *mapping = inverseMap(point);
  *normal = *vertexNormal1 * (*mapping)[0] +
            *vertexNormal2 * (*mapping)[1] +
